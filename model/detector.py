...
class UNIK(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.mean(4).mean(1) # N C T'
        x = F.upsample(x, T, mode='linear') # N C T
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_joints=25, num_person=2, tau=1, num_heads=3, in_channels=2, weights = 'no'):
        ...
        self.unik = UNIK(320, num_joints, num_person, tau, num_heads, in_channels)
        if weights!='no':
            ...
            try:
                self.unik.load_state_dict(weights)
            except:
                state = self.unik.state_dict()
                diff = list(set(state.keys()).difference(set(weights.keys())))
                print('Can not find these weights:')
                for d in diff:
                    print('  ' + d)
                state.update(weights)
                self.unik.load_state_dict(state)

        self.pdan = Dilated_TCN(inter_dim=512, input_dim=256, num_classes=num_class)
        self.unik.fc = nn.Linear(256, num_class)
        nn.init.normal_(self.unik.fc.weight, 0, math.sqrt(2. / num_class))

   

    def forward(self, x, mask_new):
        N, C, T, V, M = x.size()
        max_len = 0
