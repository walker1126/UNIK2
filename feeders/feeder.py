...
class Feeder_det(Dataset):
    ...
    def __len__(self):
        return len(self.label)
        #return int(len(self.label) * 0.05)
    ...
    def __getitem__(self, index):
        ...
        duration = self.label[index][1]
        vid = self.sample_name[index]        

        data_numpy = np.array(data_numpy)
        T = int(data_numpy[-1,-1,-1,-1])
        if data_numpy.shape[3]==1:
            data_numpy = np.concatenate((data_numpy[:,:,:,:], np.zeros((data_numpy.shape))), axis=3)
        if self.normalization:
            data_numpy = (data_numpy - self.mean_map) / self.std_map
        if self.random_shift:
            data_numpy = tools.random_shift(data_numpy)
        if self.random_choose:
            data_numpy, label, T = tools.random_choose_pku(data_numpy, label, self.window_size)
            data_numpy[-1,-1,-1,-1] = T
        elif self.window_size > 0:
            data_numpy = tools.auto_pading(data_numpy, self.window_size)
        if self.random_move:
            data_numpy = tools.random_move(data_numpy)
            data_numpy[-1,-1,-1,-1] = T
        mask = np.zeros((data_numpy.shape[1]), np.float32)
        if self.random_choose:
            
            mask[:T] = 1
        else:
            mask[:int(data_numpy[-1,-1,-1,-1])] = 1 

        return data_numpy, mask, label, [vid, duration, index]
